...
cmake .. -DCMAKE_BUILD_TYPE=Release -DPORTABLE=ON -DUSE_NCNN=OFF -DUSE_TRT=OFF -DUSE_ORT=ON \
         -DONNXRUNTIME_ROOT="$ORT_ROOT" 2>&1 | tee configure.log | grep -iE "backend:|error" || true
cmake --build . --parallel "$(nproc)" 2>&1 | tee build.log | grep -iE "error|Built target" | tail -2 || true

BIN="$ROOT/cpp/build_ort_trt/yolomaster_edge"
[ -x "$BIN" ] || { echo "build completed without an executable: $BIN (see build_ort_trt/build.log)" >&2; exit 1; }
echo
echo "built: $BIN"

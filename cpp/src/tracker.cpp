...
struct KF {
    ...
    cv::Rect2f box() const {
        if (xywh) return cv::Rect2f(static_cast<float>(mean[0] - mean[2] / 2), static_cast<float>(mean[1] - mean[3] / 2),
                                    static_cast<float>(mean[2]), static_cast<float>(mean[3]));
        const double w = mean[2] * mean[3];
        return cv::Rect2f(static_cast<float>(mean[0] - w / 2), static_cast<float>(mean[1] - mean[3] / 2),
                          static_cast<float>(w), static_cast<float>(mean[3]));
    }
    std::array<double, NM> measure(const cv::Rect2f& b) const {
...
struct Tracker::Impl {
    ...
    bool estimate_motion(const cv::Mat& frame, double R[2][2], double& tx, double& ty) {
        R[0][0] = R[1][1] = 1; R[0][1] = R[1][0] = 0; tx = ty = 0;
#ifdef HAVE_GMC
        cv::Mat gray;
        ...
        if (!prev_gray.empty() && prev_gray.size() == gray.size() && prev_pts.size() >= 8) {
            ...
            if (a.size() >= 8) {
                ...
                if (!H.empty() && H.rows == 2 && H.cols == 3) {
                    R[0][0] = H.at<double>(0, 0); R[0][1] = H.at<double>(0, 1);
                    R[1][0] = H.at<double>(1, 0); R[1][1] = H.at<double>(1, 1);
                    tx = H.at<double>(0, 2) * kDownscale; ty = H.at<double>(1, 2) * kDownscale;
                    ok = true;
                }
            }
        }
    ...
    static std::vector<std::vector<double>> cost_matrix(const std::vector<STrack*>& trks, const std::vector<const Detection*>& dets,
                                                        bool fuse) {
        std::vector<std::vector<double>> c(trks.size(), std::vector<double>(dets.size(), 1.0));
        for (size_t i = 0; i < trks.size(); ++i) {
            const cv::Rect2f tb = trks[i]->kf.box();
            for (size_t j = 0; j < dets.size(); ++j) {
                double s = iou(tb, dets[j]->box);
    ...
    void activate(STrack& t, const Detection& d, int det_index) {
        t.id = next_id++;
        t.kf.xywh = (cfg.kind == TrackerConfig::Kind::BotSort);
    ...
    void update_track(STrack& t, const Detection& d, int det_index, bool reactivate) {
    ...
    std::vector<Track> update(const std::vector<Detection>& dets, const cv::Mat* frame) {
        ...
        if (cfg.kind == TrackerConfig::Kind::BotSort && cfg.gmc && frame && !frame->empty()) {
            double R[2][2], tx, ty;
            if (estimate_motion(*frame, R, tx, ty)) {
                gmc_last = true;
                for (auto* t : pool) t->kf.apply_affine(R, tx, ty);
                for (auto* t : unconfirmed) t->kf.apply_affine(R, tx, ty);
            }
        }
        ...
        std::vector<const Detection*> rem_high; std::vector<int> rem_idx;
...
int Tracker::frame_count() const { return impl_->frame_id; }

...
**Stage 1 (this build):** single-image inference, live conf/IoU tuning without
re-inferring ("forward once, tune cheap"), box styles (hud / solid / neon), label
modes, per-class counts, timing stats. CPU only. Detection models.

**Later:** folder batch, video, webcam, segmentation overlays, stretch preprocess,
CUDA.
...
2. **Open image...** — pick a `.jpg/.png`. Inference runs automatically.

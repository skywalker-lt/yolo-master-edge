...
final class CameraController: NSObject, ObservableObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    @Published var candidates: [Detection] = []   // pre-NMS, camera-pixel coords; overlay applies conf/iou
    ...
    func stop() {
        camQueue.async { [weak self] in
            ...
            DispatchQueue.main.async { self.running = false; self.candidates = []; self.latencyMs = 0; self.fps = 0 }
        }
    }
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        do {
            ...
            let cands = det.candidates(raw, confFloor: 0.05)
...
struct LiveCameraView: View {
    ...
    var body: some View {
        CameraStage(cam: cam, conf: conf, iou: iou, style: style, label: label)
...
struct CameraStage: View {
    ...
    let style: BoxStyle, label: LabelMode
    var body: some View {
        let dets = Detector.nms(cam.candidates, conf: Float(conf), iou: CGFloat(iou))
